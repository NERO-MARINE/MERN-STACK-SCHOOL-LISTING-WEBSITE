import { Link } from "react-router-dom";

export const userColumns = [
  { field: "_id", headerName: "ID", width: 70 },
  { field: "username", headerName: "Username", width: 120 },
  { field: "email", headerName: "Email", width: 230 },
  { field: "state", headerName: "State", width: 100 },
  { field: "lga", headerName: "Lga", width: 160 },
];

// rows for userColumns(userRows) have been replaced with data from our api

/*
export const userRows = [
  {
    id: 1,
    username: "Snow",
    image:
      "data:image/jpeg;base64,",
    email: "snow@gmail.com",
    status: "active",
    age: 35,
  },
  {
    id: 2,
    username: "yoko",
    image:
      "data:image/jpeg;base64,",
    email: "yoko@gmail.com",
    status: "passive",
    age: 20,
  },
  {
    id: 3,
    username: "Favoryt",
    image:
      "data:image/jpeg;base64,",
    email: "fav@gmail.com",
    status: "pending",
    age: 19,
  },
  {
    id: 4,
    username: "Milo",
    image:
      "data:image/jpeg;base64,",
    email: "milo@gmail.com",
    status: "active",
    age: 22,
  },

  {
    id: 5,
    username: "Mimi",
    image:
      "data:image/jpeg;base64,",
    email: "mimi@gmail.com",
    status: "passive",
    age: 15,
  },
  {
    id: 6,
    username: "Nero",
    image:
      "data:image/jpeg;base64,",
    email: "nero@gmail.com",
    status: "active",
    age: 38,
  },
  {
    id: 7,
    username: "Doe",
    image:
      "data:image/jpeg;base64,",
    email: "Doe@gmail.com",
    status: "active",
    age: 35,
  },
  {
    id: 8,
    username: "koko",
    image:
      "data:image/jpeg;base64,",
    email: "koko@gmail.com",
    status: "pending",
    age: 21,
  },
  {
    id: 9,
    username: "Tavoryt",
    image:
      "data:image/jpeg;base64,",
    email: "tvoryt@gmail.com",
    status: "pending",
    age: 19,
  },
  {
    id: 10,
    username: "klilo",
    image:
      "data:image/jpeg;base64,",
    email: "kililo@gmail.com",
    status: "passive",
    age: 22,
  },

  {
    id: 11,
    username: "Kelly",
    image:
      "data:image/jpeg;base64,",
    email: "kelly@gmail.com",
    status: "active",
    age: 13,
  },
  {
    id: 12,
    username: "Vero",
    image:
      "data:image/jpeg;base64,",
    email: "vero@gmail.com",
    status: "active",
    age: 31,
  },
];
*/

export const schoolColumns = [
  { field: "_id", headerName: "ID", width: 70 },
  { field: "name", headerName: "School Name", width: 120 },
  { field: "email", headerName: "Email", width: 100 },
  { field: "phone", headerName: "Phone No", width: 100 },
  { field: "state", headerName: "State", width: 100 },
  { field: "lga", headerName: "LGA", width: 100 },
  {
    field: "googleProfile",
    headerName: "Google Profile",
    width: 100,
    renderCell: (params) => {
      return (
        <div className="">
          <Link
            to={`https://${params.row.googleProfile}`}
            style={{ textDecoration: "none" }}
          >
            <div>{params.row.googleProfile}</div>
          </Link>
        </div>
      );
    },
  },

  { field: "featured", headerName: "Featured", width: 100 },
  { field: "approved", headerName: "approved", width: 160 },
];

// rows for schoolColumns will be gotton from our api
